...
[VALIDATION GATE: researcher reviews parser + gap outputs]
    |
    v
Layer 3: Body-region summaries + frame/window warnings       [future, do not implement yet]
    |
    v
Layer 4: Artifact candidate screening                        [future, do not implement yet]
    |
    v
Layer 5: Publication-ready report package                    [future, do not implement yet]
    |
    v
...
| `motive_raw_qc.py` | Layer 1-2 backend implementation for initial version | Yes |
...
artifacts:
  enabled: false  # Layer 4 only. Do not implement in Layers 1-2.
  methods:
    velocity_mad: true
...
outputs:
  ...
  plots:
    ...
    artifact_timeline: false  # Layer 4 only.
...
**Future layer. Do not implement yet.**

### Goal

Make the QC report useful for later PCA/jPCA/jcvPCA window selection by identifying risky frames and fixed-duration windows.
...
Identify potential spikes, jumps, marker swaps, constant-position holds, and extreme velocity/acceleration events without overclaiming.
...
│   └── missing_data_heatmap_unlabeled.png
├── qc_report_summary.txt
├── qc_report.xlsx
└── config_used.yaml
```
...
**Do now:** parse, inventory, quantify missingness, detect gaps, write tables/plots/workbook.  
**Do not do yet:** frame/window warnings, artifact detection, filtering, smoothing, interpolation, BVH parsing, PCA, final report generation.
