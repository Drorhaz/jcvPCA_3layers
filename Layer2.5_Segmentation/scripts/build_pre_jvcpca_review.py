...
from pre_jvcpca_review.export_window import export_pilot_window_for_jvcpca, export_window_for_jvcpca
...
def main() -> None:
    ...
    mode.add_argument(
        "--pilot-export",
        action="store_true",
        help="Export using frozen Group4 upper-body pilot manifest with validation gate",
    )
    ...
    parser.add_argument(
        ...
        help="Pilot feature manifest CSV (for --pilot-export)",
    )
    parser.add_argument(
        "--qc-evidence",
    ...
    if args.export_window_only or args.pilot_export:
        if args.start_frame is None or args.end_frame is None:
            parser.error("--start-frame and --end-frame are required for export modes")

        if args.pilot_export:
            ...
            selected = _parse_list(args.selected_links)
            if not selected:
                parser.error("--selected-links is required unless --mapping-only or --pilot-export")
