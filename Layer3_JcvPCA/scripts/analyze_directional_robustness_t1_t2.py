#!/usr/bin/env python3
"""Directional robustness of T1 vs T2 link-level JcvPCA across repetition pairings."""
...
POOLED_VARIANT_ID = "DR_pooled_T1_vs_T2"
POOLED_COMPARISON_LABEL = "T1_vs_T2"
...
VARIANTS: list[VariantSpec] = [
    VariantSpec("DR_T1R1_vs_T2R1", "T1_R1_vs_T2_R1", ("R1",), ("R1",)),
    VariantSpec("DR_T1R2_vs_T2R2", "T1_R2_vs_T2_R2", ("R2",), ("R2",)),
    VariantSpec(POOLED_VARIANT_ID, POOLED_VARIANT, ("R1", "R2"), ("R1", "R2"), use_batch=True),
    VariantSpec("DR_T1R1_vs_T2R2", "T1_R1_vs_T2_R2", ("R1",), ("R2",)),
    VariantSpec("DR_T1R2_vs_T2R1", "T1_R2_vs_T2_R1", ("R2",), ("R1",)),
]
...
def build_variant_from_pooled_construction(
    ...
    """Build filtered datasets from pooled T1_vs_T2 construction and run JcvPCA workbench."""
...
def ensure_variant_run(
    ...
    if variant.use_batch:
        comp_key = f"{participant_id}_{block_id}_L1_T1_vs_T2"
...
def main() -> None:
    ...
    if not link_out.empty:
        ...
        link_out.to_csv(BATCH / "directional_robustness_t1_t2_link_results.csv", index=False)

    summary = pd.DataFrame(summary_rows)
    if not summary.empty:
        summary.to_csv(BATCH / "directional_robustness_t1_t2_summary.csv", index=False)
    ...
    if not top.empty:
        top.to_csv(BATCH / "directional_robustness_t1_t2_top_links.csv", index=False)

    print(f"Directional robustness T1 vs T2 analysis complete: {BATCH}")
