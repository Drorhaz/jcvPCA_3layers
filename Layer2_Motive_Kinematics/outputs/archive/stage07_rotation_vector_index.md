# Stage 07 rotation-vector batch index

Batch index of per-file relative quaternion → rotation-vector log-map conversion.

## Explicit limitation

Stage 07 rotation-vector success does not filter, finalize analysis features, resolve skeleton-version mismatch, or make Layer 3 ready.

Core and excluded link diagnostics are interpreted separately. Excluded distal/finger/toe warnings do not block Stage 08 authorization when core candidate links pass.

Total runs: 6

## `671_T1_P1_R1_Take_2026-01-06_03.57.12_PM_001`

- **Input:** `/Users/drorhazan/Desktop/gaga_psilo/projects/3Layers_project/Layer2_Motive_Kinematics/data/671/671_T1_P1_R1_Take 2026-01-06 03.57.12 PM_001.csv`
- **Output:** `outputs/671_T1_P1_R1_Take_2026-01-06_03.57.12_PM_001`
- **Links processed:** 50 (core/review/excluded: 16/2/32)
- **Max rotvec norm (core / all):** 2.6809826243076738 / 2.6809826243076738 rad
- **Max jump (core / all):** 1.4740064916106252 / 2.070339010611674 rad
- **Core warnings/failures:** 2/1
- **Review warnings/failures:** 0/0
- **Excluded warnings/failures:** 8/8
- **Stage 08 may proceed:** False
- **Limitations:** Stage 07 rotation-vector success does not filter, finalize analysis features, resolve skeleton-version mismatch, or make Layer 3 ready.

## `671_T1_P1_R2_Take_2026-01-06_03.57.12_PM_003`

- **Input:** `/Users/drorhazan/Desktop/gaga_psilo/projects/3Layers_project/Layer2_Motive_Kinematics/data/671/671_T1_P1_R2_Take 2026-01-06 03.57.12 PM_003.csv`
- **Output:** `outputs/671_T1_P1_R2_Take_2026-01-06_03.57.12_PM_003`
- **Links processed:** 50 (core/review/excluded: 16/2/32)
- **Max rotvec norm (core / all):** 2.646642894157299 / 2.646642894157299 rad
- **Max jump (core / all):** 0.358889677088691 / 0.358889677088691 rad
- **Core warnings/failures:** 0/0
- **Review warnings/failures:** 0/0
- **Excluded warnings/failures:** 0/0
- **Stage 08 may proceed:** True
- **Limitations:** Stage 07 rotation-vector success does not filter, finalize analysis features, resolve skeleton-version mismatch, or make Layer 3 ready.

## `671_T2_P1_R1_Take_2026-01-15_04.35.25_PM_005`

- **Input:** `/Users/drorhazan/Desktop/gaga_psilo/projects/3Layers_project/Layer2_Motive_Kinematics/data/671/671_T2_P1_R1_Take 2026-01-15 04.35.25 PM_005.csv`
- **Output:** `outputs/671_T2_P1_R1_Take_2026-01-15_04.35.25_PM_005`
- **Links processed:** 50 (core/review/excluded: 16/2/32)
- **Max rotvec norm (core / all):** 2.4743873928737568 / 2.4743873928737568 rad
- **Max jump (core / all):** 0.7187676308631565 / 1.9718061304534047 rad
- **Core warnings/failures:** 2/0
- **Review warnings/failures:** 0/0
- **Excluded warnings/failures:** 5/6
- **Stage 08 may proceed:** True
- **Limitations:** Stage 07 rotation-vector success does not filter, finalize analysis features, resolve skeleton-version mismatch, or make Layer 3 ready.

## `671_T2_P1_R2_Take_2026-01-15_04.35.25_PM_009`

- **Input:** `/Users/drorhazan/Desktop/gaga_psilo/projects/3Layers_project/Layer2_Motive_Kinematics/data/671/671_T2_P1_R2_Take 2026-01-15 04.35.25 PM_009.csv`
- **Output:** `outputs/671_T2_P1_R2_Take_2026-01-15_04.35.25_PM_009`
- **Links processed:** 50 (core/review/excluded: 16/2/32)
- **Max rotvec norm (core / all):** 2.6632303132920425 / 2.6632303132920425 rad
- **Max jump (core / all):** 0.3408985148927828 / 0.3985822210178982 rad
- **Core warnings/failures:** 0/0
- **Review warnings/failures:** 0/0
- **Excluded warnings/failures:** 0/0
- **Stage 08 may proceed:** True
- **Limitations:** Stage 07 rotation-vector success does not filter, finalize analysis features, resolve skeleton-version mismatch, or make Layer 3 ready.

## `671_T3_P1_R1_Take_2026-02-03_08.05.01_PM_000`

- **Input:** `/Users/drorhazan/Desktop/gaga_psilo/projects/3Layers_project/Layer2_Motive_Kinematics/data/671/671_T3_P1_R1_Take 2026-02-03 08.05.01 PM_000.csv`
- **Output:** `outputs/671_T3_P1_R1_Take_2026-02-03_08.05.01_PM_000`
- **Links processed:** 54 (core/review/excluded: 16/6/32)
- **Max rotvec norm (core / all):** 2.690689758744655 / 2.690689758744655 rad
- **Max jump (core / all):** 1.7399371866532016 / 2.0068197335255222 rad
- **Core warnings/failures:** 0/3
- **Review warnings/failures:** 0/0
- **Excluded warnings/failures:** 7/7
- **Stage 08 may proceed:** False
- **Limitations:** Stage 07 rotation-vector success does not filter, finalize analysis features, resolve skeleton-version mismatch, or make Layer 3 ready.

## `671_T3_P1_R2_Take_2026-02-03_08.05.01_PM_005`

- **Input:** `/Users/drorhazan/Desktop/gaga_psilo/projects/3Layers_project/Layer2_Motive_Kinematics/data/671/671_T3_P1_R2_Take 2026-02-03 08.05.01 PM_005.csv`
- **Output:** `outputs/671_T3_P1_R2_Take_2026-02-03_08.05.01_PM_005`
- **Links processed:** 54 (core/review/excluded: 16/6/32)
- **Max rotvec norm (core / all):** 2.6453381812844876 / 2.6453381812844876 rad
- **Max jump (core / all):** 0.8182085932058963 / 0.8182085932058963 rad
- **Core warnings/failures:** 1/0
- **Review warnings/failures:** 0/0
- **Excluded warnings/failures:** 7/0
- **Stage 08 may proceed:** True
- **Limitations:** Stage 07 rotation-vector success does not filter, finalize analysis features, resolve skeleton-version mismatch, or make Layer 3 ready.
